# Re-Frame Default README for My Own Reference
...
* [`src/skill_match/`](src/skill_match/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/skill_match/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `skill-match` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/skill_match/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns skill-match.example
  (:require [skill-match.config :as config])
